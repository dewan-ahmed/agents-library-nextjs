"use client";
import { useMemo, useState, type FormEvent, useContext } from "react";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import { agents, pipelines, yamlForPipeline } from "@/lib/catalog";
import { AgentCard } from "@/components/Cards";
import { CopyButton } from "@/components/YamlBlock";
import { matchCatalog, starterPrompts } from "@/lib/match";
import { applyYamlScope, type HarnessScope } from "@/lib/scope";
import { ScopeContext } from "./layout";
...
function HomePageInner() {
  ...
  function submit(event?: FormEvent) {
    ...
    router.push(draft.trim() ? `/?${next.toString()}` : "/");
  }
      ...
      <section className="hero">
        ...
        <form className="composer" onSubmit={submit}>
          ...
          <div className="composer-row">
            ...
            <button className="btn" type="submit">Match agents</button>
          </div>
        </form>
        ...
        <div className="section">
          ...
          <div className="grid grid-3">
            ...
            <div className="card">
              ...
              <Link href="/pipelines">Browse all pipelines</Link>
            </div>
            <div className="card">
              <h3>Lifecycle</h3>
              <p>See how many agents contribute to build, deploy, secure, and the rest of the delivery cloud.</p>
              <Link href="/lifecycle">Open the lifecycle cloud</Link>
            </div>
          </div>
      ...
      <div className="grid">
        {result.agents.map(({ agent, reasons }) => (
          <article className="card" key={agent.id}>
            ...
            <h3><Link href={`/agents/${agent.id}`}>{agent.name}</Link></h3>
            <p>{agent.summary}</p>
            <div className="starters">
        ...
        {result.pipelines.map(({ pipeline, reasons }) => (
          <article className="card" key={pipeline.id}>
            ...
            <div className="card-foot">
              <Link href={`/pipelines/${pipeline.id}`}>View example</Link>
