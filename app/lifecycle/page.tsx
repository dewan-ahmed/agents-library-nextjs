...
import {
  agents, agentsGroupedByLifecycle, customAgents, marketplaceAgents, type Agent,
} from "@/lib/catalog";
...
type Bubble = {
  key: string; label: string; color: string; count: number; agents: Agent[];
...
function packBubbles(groups: ReturnType<typeof agentsGroupedByLifecycle>): Bubble[] {
  ...
  const seeded = groups.map((g) => ({ ...g, r: 34 + Math.sqrt(g.count / max) * 78, x: 0, y: 0 }));
  const placed: Bubble[] = [];
  for (const bubble of seeded) {
    if (placed.length === 0) { placed.push(bubble); continue; }
    ...
    <div className="page page-wide">
      ...
      <div className="starters" style={{ marginTop: 18 }}>
        {([
          ...
          <button key={key} type="button" className={`chip ${source === key ? "active" : ""}`}
            onClick={() => { setSource(key); setSelected(null); }}>
            {label}
          </button>
        ))}
      </div>
      <div className="life-cloud" aria-label="Agent contribution cloud by lifecycle">
        ...
        <svg className="life-svg" viewBox={`${minX} ${minY} ${maxX - minX} ${maxY - minY}`} role="img">
          ...
          {bubbles.map((bubble, index) => {
            ...
            return (
              <g key={bubble.key} transform={`translate(${bubble.x} ${bubble.y})`}>
                <g className={`life-bubble ${isOn ? "is-on" : ""}`}
                  style={{ animationDelay: `${index * 0.28}s` }}
                  role="button" tabIndex={0} aria-pressed={isOn}
                  aria-label={`${bubble.label}, ${bubble.count} agents`}
                  onClick={() => setSelected(bubble.key)}
                  onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setSelected(bubble.key); } }}>
                  <circle r={bubble.r} fill={bubble.color} fillOpacity={isOn ? 0.96 : 0.8} />
                  <circle r={bubble.r * 0.7} fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.32)" strokeWidth={1.5} />
                  <text className="life-word" y={-6} fontSize={wordSize}>{bubble.label}</text>
                  <text className="life-count" y={countSize - 4} fontSize={countSize}>{bubble.count}</text>
                </g>
              </g>
            );
          })}
      ...
      {active && (
        <div className="section">
          <h2>{active.label} <span className="muted" style={{ marginLeft: 10, fontWeight: 500 }}>{active.count} {active.count === 1 ? "agent" : "agents"}</span></h2>
          <div className="grid">
            {active.agents.map((agent) => <AgentCard key={agent.id} agent={agent} />)}
          </div>
          <p className="muted" style={{ marginTop: 16 }}>
            <Link href="/agents">Browse the full catalog</Link> and filter by lifecycle there.
          </p>
        </div>
      )}
    </div>
